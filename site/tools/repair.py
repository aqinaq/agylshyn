...
# ---------------------------------------------------------------------------
# Collins: digits that OCR'd as letters
# ---------------------------------------------------------------------------

# The two Collins books were OCR'd from a scan whose digits and letters run
# together: "Exercise 4" came out as "Exercise U", "4 warned" as "U warned",
# "5g" as "Sg". The exercise labels are the visible half of it — a reader sees
# "Vocabulary · Exercise U" where the book prints 4.
#
# Neither is guessed from a confusion table, because the same misread letter
# stands for different digits in different books. Both are read off the
# surrounding sequence instead: an exercise between 3 and 5 is 4 whatever the
# scanner made of it.

_EX_LABEL = re.compile(r'^(?P<head>.*?Exercise\s+)(?P<label>\S+)\s*$')


def renumber_exercises(units):
    """Give an unreadable exercise label the number its neighbours imply.

    Numbering restarts inside each named section ("Vocabulary", "Practice
    exercises"), so the sequence is tracked per section. A label is only
    rewritten when the number it would take still leaves room for the next
    readable label — otherwise the scan lost a whole exercise and guessing
    would put every later one out by one."""
    fixed = 0
    for u in units:
        subs = u.get('subExercises', []) or []
        labels = []
        for s in subs:
            m = _EX_LABEL.match(s.get('number') or '')
            labels.append(m)
        for i, (s, m) in enumerate(zip(subs, labels)):
            if not m or m.group('label').isdigit():
                continue
            section = (s.get('number') or '').split('·')[0]
            # the last readable number in this section, before this one
            prev = 0
            for j in range(i - 1, -1, -1):
                if labels[j] and (subs[j].get('number') or '').split('·')[0] == section:
                    if labels[j].group('label').isdigit():
                        prev = int(labels[j].group('label'))
                        break
            want = prev + 1
            # the next readable number in this section has to leave room
            nxt = None
            for j in range(i + 1, len(subs)):
                if labels[j] and (subs[j].get('number') or '').split('·')[0] == section:
                    if labels[j].group('label').isdigit():
                        nxt = int(labels[j].group('label'))
                        break
            if nxt is not None and nxt <= want:
                continue
            s['number'] = m.group('head') + str(want)
            fixed += 1
    return fixed


def fix_key_numbering(text):
    """Restore a list marker the scan turned into a letter.

    Only where the surrounding run proves it: "3 recommended, U warned, 5
    persuaded" is missing exactly one number between two present ones, and holds
    exactly one lone letter where it should be. A run that is ambiguous, or a
    lone letter that is itself an answer ("3 a, 4 b"), is left as it is."""
    if not isinstance(text, str) or not text:
        return text
    marks = [(int(m.group(1)), m.start(1), m.end(1))
             for m in re.finditer(r'(?:^|[\s,;.])(\d{1,2})\s+\S', text)]
    out = text
    for (a, _, a_end), (b, b_start, _) in zip(marks, marks[1:]):
        if b != a + 2:                     # exactly one number missing
            continue
        segment = out[a_end:b_start]
        lone = re.findall(r'(?:^|[\s,;])([A-Za-z])(?=\s)', segment)
        if len(lone) != 1:
            continue
        m = re.search(r'(^|[\s,;])(%s)(?=\s)' % re.escape(lone[0]), segment)
        out = out[:a_end] + segment[:m.start(2)] + str(a + 1) + segment[m.end(2):] + out[b_start:]
    return out


def fix_collins_ocr(units):
    """Both Collins repairs. Returns (labels_fixed, keys_fixed)."""
    labels = renumber_exercises(units)
    keys = 0
    for u in units:
        for s in u.get('subExercises', []) or []:
            if s.get('rawAnswer'):
                fixed = fix_key_numbering(s['rawAnswer'])
                if fixed != s['rawAnswer']:
                    s['rawAnswer'] = fixed
                    keys += 1
    return labels, keys
